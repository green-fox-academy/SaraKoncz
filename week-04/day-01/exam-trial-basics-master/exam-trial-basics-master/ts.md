...
#### Your answer: The Math.random() function returns a (pseudo-)random number which is equal or grather than 0 and smaller than 1. If we want to have this random number between two numbers (min and max) we schould use it as follows: return Math.random() * (max - min) + min;
